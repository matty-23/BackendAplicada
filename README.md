...
1. [Módulo de Notificaciones (`/notificaciones`)](#1-módulo-de-notificaciones)
2. [Módulo de Eventos (`/api/Eventos`)](#2-módulo-de-eventos)
3. [Módulo de Solicitudes (`/api/solicitudes`)](#3-módulo-de-solicitudes)
4. [Módulo de Usuarios (`/api`)](#4-módulo-de-usuarios)
5. [Tabla Resumen de Rutas](#5-tabla-resumen-de-rutas)
...
**Guards Globales del Controlador**: `AuthGuard`, `PermissionsGuard`
...
- **Descripción**: Envía un correo electrónico notificando la confirmación de una solicitud para un evento. Si falla el envío, retorna un error HTTP 403 Forbidden.
...
- **Cuerpo de la petición (`Body`)**: `CorreoDTO`
- **Respuesta**: `200 OK` (Vacío)
- **Descripción**: Envía un correo con notificaciones generales del sistema a un usuario determinado.
...
**Guards Globales del Controlador**: `AuthGuard`
...
- **Parámetros de Ruta (`Param`)**:
  - `page` (number, obligatorio): Número de página (convertido numéricamente por `ParseIntPipe`).
...
- **Descripción**: Retorna los detalles de un evento en específico. Si no se encuentra, retorna HTTP 404.
...
- **Cuerpo de la petición (`Body`)**: `string[]` (Arreglo con los IDs de los eventos a eliminar).
- **Respuesta**: `200 OK` (Vacío)
- **Descripción**: Elimina del sistema una lista de eventos recibida por Body. Si falla, lanza un HTTP 404.
...
- **Permisos requeridos**: `MODIFICAR_EVENTOS`
- **Cuerpo de la petición (`Body`)**: `string[]` (Arreglo con los IDs de los participantes a agregar).
...
- **Descripción**: Obtiene las solicitudes de reserva/eventos creadas por el usuario activo en la sesión (`req.user.id`).
...
- **Descripción**: Permite a los usuarios administradores/gestores explorar y buscar entre todas las solicitudes registradas.
...
- **Descripción**: Obtiene los detalles completos de una solicitud. Si no se encuentra, arroja HTTP 404.
...
- **Descripción**: Desestima/rechaza una solicitud especificando opcionalmente el motivo del rechazo.
...
- **Descripción**: Obtiene la lista de usuarios. Devuelve la información de paginación en las cabeceras HTTP (`headers`).
...
- **Permisos requeridos**: Se requiere al menos uno de los siguientes:
  ...
  - `MODIFICAR_ROL`: Requerido si el cuerpo de la petición incluye la modificación del rol.
- **Cuerpo de la petición (`Body`)**: `ActualizarUsuarioDTO`
- **Respuesta**: `200 OK` - Objeto `ObtenerUsuarioDTO` actualizado.
- **Descripción**: Modifica parcialmente los atributos de un usuario. Aplica validaciones de seguridad para restringir cambios de rol o modificación de terceros si no posee los permisos adecuados.
...
- **Cuerpo de la petición (`Body`)**: `ActualizarUsuarioCompletoDTO`
...
- **Respuesta**: `200 OK` - `boolean` (`true` si fue exitoso).
...
Aquí tenés el archivo `README.md` generado con la documentación completa de todas las rutas de la API, organizadas por módulo, con sus respectivos métodos HTTP, parámetros, cuerpos de solicitud (`Body`), permisos requeridos y guardias de seguridad.

Tu archivo Markdown está listo:
[file-tag: code-generated-file-c5713b48-6b01-4d52-98f1-6d0ac7e91278]
