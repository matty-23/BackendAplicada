...
export class EventoController {
    ...
    async getAll(@Param('page') page: number) {
        ...
        return await Promise.all(eventos.map(e => this.mapearEventoADto(e)));
    }
    @Get('filtros')
    ...
    // POST /api/Eventos/mono
    @Post('mono')
    @HttpCode(201)
    async registrarMono(@Body() dto: CrearEventoMonoDTO) {
        const categoria = dto.categoria || 'sin_categoria';

        // Se crea la única ocurrencia
        const ocurrencia = new Ocurrencia(
            '0',
            '0',
            new Date(dto.ocurrencia.fechaInicio),
            new Date(dto.ocurrencia.fechaFinalizacion),
            dto.ocurrencia.lugar,
            dto.ocurrencia.cantidadPersonas
        );

        // Se instancia el evento con un array de 1 sola ocurrencia
        const evento = new Evento('0', dto.titulo, 'pendiente', categoria, [ocurrencia]);

        // Ambos llaman a addEvento
        const eventoRes = await this._eventoService.addEvento(evento);
        if (!eventoRes) throw new BadRequestException("Error al registrar el Evento Mono-día.");
